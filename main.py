...
def normal_model(X_train_scaled,X_test_scaled,y_train,y_test):
    ...
    return normal_df,bt_collection,results,y_pred_dict
...
def grid_model(X_train_scaled,X_test_scaled,y_train,y_test):
    ...
    return grid_df,bt_collection,results,y_pred_dict
...
def random_model(X_train_scaled,X_test_scaled,y_train,y_test):
    ...
    return random_df,bt_collection,results,y_pred_dict
...
if __name__ == "__main__":
    ...
    for filepath in os.listdir(dir):
        ...
        if filepath.endswith('.csv'):
            ...
            saving_dir = os.path.join(dir, 'normal') 
            ...
            #GridSearchCV
            grid_df,bt_collection,results,y_pred_dict = grid_model(X_train_scaled,X_test_scaled,y_train,y_test)
            saving_dir = os.path.join(dir, 'grid')
            os.makedirs(saving_dir,exist_ok=True)
            print("Saving Backtesting Result")
            ...
            #RandomSearchCV
            random_df,bt_collection,results,y_pred_dict = random_model(X_train_scaled,X_test_scaled,y_train,y_test)
            saving_dir = os.path.join(dir, 'random')
            os.makedirs(saving_dir,exist_ok=True)
            print("Saving Backtesting Result")
